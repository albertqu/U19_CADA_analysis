...
class PoseSeries:
    ...
    def __init__(self, root, animal, session):
        ...
        for r in range(1, 5):
            track_f = oj(
                track_session_root, f"{animal}_{session}_R{r}_tracks_processed.csv"
            )
    ...
    def summarize_sleap_info(self):
        ...
        sleap_tdf = (
            track_df.loc[
                track_df["sleap_event"] == "T_Entry",
                ["trial", "time", "rel_time", "sleap_event", "restaurant"],
            ]
            .sort_values(["trial", "rel_time"])
            .reset_index(drop=True)
        )
        sleap_tdf.rename(
            columns={"time": "slp_T_Entry_time", "rel_time": "slp_hall_time"},
            inplace=True,
        )
        sleap_tdf["slp_tone_onset_time"] = (
            sleap_tdf["slp_T_Entry_time"] - sleap_tdf["slp_hall_time"]
        )
        dfa = track_df.loc[
            track_df["sleap_event"] == "ACC",
            ["trial", "time", "rel_time", "sleap_event"],
        ].sort_values(["trial", "rel_time"])
        dfb = track_df.loc[
            track_df["sleap_event"] == "REJ",
            ["trial", "time", "rel_time", "sleap_event"],
        ].sort_values(["trial", "rel_time"])
        ...
        dfd = (
            track_df.loc[
                track_df["sleap_event"] == "quit", ["trial", "time", "rel_time"]
            ]
            .sort_values(["trial", "rel_time"])
            .reset_index(drop=True)
        )
        ...
        sleap_tdf = pd.merge(
            sleap_tdf, dfd[["trial", "slp_quit_time"]], on="trial", how="left"
        )
        ...
        sleap_tdf["slp_decision"] = (
            sleap_tdf["slp_accept"].astype(float).map({1.0: "accept", 0.0: "reject"})
        )
    ...
    def preprocess_track_df(self, resample_interval=0.05):
        ...
        def resamp_track_df_calc_v(tdf, meas_cols, resample_interval=0.05):
            ...
            for i in range(len(meas_cols)):
                ...
                new_df[f"{pream}_vel_{parts[-1]}"] = (
                    np.gradient(ynew) * 1 / resample_interval
                )  # using bspline derivative
            return new_df

        track_df = self.track_df
        pre_tdf = track_df[
            ["trial", "time", "rel_time"] + list(track_df.columns[8:16])
        ].reset_index(drop=True)
        resamp_track_df = (
            pre_tdf.groupby("trial", as_index=True)
            .apply(
                resamp_track_df_calc_v,
                meas_cols=track_df.columns[8:16],
