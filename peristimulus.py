...
def align_activities_with_event(sigs, times, event_times, time_window, discrete=True):
    ...
    if discrete:
        ...
        if len(sigs.shape) != 1:
            ...
            result = np.full((k, len(time_window)))
            for ik in range(k):
                if len(event_times.shape) > len(sigs.shape):
                    evnt = event_times[:, ik]
                else:
                    evnt = event_times[np.newaxis, ik]
                rois = (times >= evnt[0] + time_window[0] - dt) & (times <= evnt[0] + time_window[-1] + dt)
...
def peristimulus_time_trial_average_plot(sigs, times, tags, extra_event_times=None, ax=None):
    ...
    for i, isig in enumerate(sigs):
        ...
        if isinstance(isig, list):
            ...
            stderr = np.nanstd(isig, axis=0) / isig.shape[0]
            ax.plot(itime, meansig, label=ilg)
            ax.fill_between(itime, meansig-stderr, meansig+stderr)
    ax.axvline(0)
