...
class PSLR_Preprocessor(CV_df_Preprocessor):
    ...
    """
    def __init__(self, lag=4, **kwargs):
    ...
    def fit_transform(self, data):
        ...
        features = ['C', 'R']
        lagfeats = list(np.concatenate([[feat + f'_{i}back' for feat in features] for i in range(1, self.lag + 1)]))

        lagdf = pd.DataFrame(lagmat(rdf[features].values, maxlag=self.lag, trim='forward', original='ex'), columns=lagfeats)
        col_keys = ['C'] + [f'C_{i}back' for i in range(1, self.lag + 1)]
        lagdf = pd.concat([rdf, lagdf], axis=1)
        idx_sel = (lagdf['Trial'] > self.lag) & np.logical_and.reduce([(lagdf[c] != -3) for c in col_keys])
