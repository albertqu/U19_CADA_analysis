...
def summarize_sessions(implant_csv_path=None, sort_key='aID'):
    ...
    """
    data = {'animal': [], 'aID': [], 'session': [], 'date': [], 'ftype': [],
            'age': [], 'FP': [], 'region': [], 'note': []}
    ...
    if implant_csv_path:
        ...
        for i in range(len(implant_df)):
            ...
            if animal_name and (str(animal_name) != 'nan'):
                ...
                implant_lookup[name_first+'_' +
                               name_sec] = {'LH': LH_target, 'RH': RH_target}
    ...
    for anim in sorted_pdf[sort_key].unique():
        ...
        sorted_pdf.loc[sorted_pdf[sort_key] == anim,
                       'S_no'] = np.arange(1, len(tempslice)+1)
    final_pdf = sorted_pdf
    final_pdf.to_csv(os.path.join(
        OUT_PATH, f"exper_list_final_{sort_key}.csv"), index=False)
        ...
        for session, session_files in grouped_by_session.items():
            ...
            for decoded_file in session_files:
                out_filepath = os.path.join(
                    session_folder, decoded_file['name'])
