...
def jove_fit_reference(reference, signal, smooth_win=10, remove=200, lambd=5e4, porder=1, itermax=50):
    '''
    Calculates z-score dF/F signal based on fiber photometry calcium-idependent
    and calcium-dependent signals
    ...
    # Remove baseline and the begining of recording
    reference = (reference[remove:] - r_base[remove:]) + np.mean(signal)
    signal = (signal[remove:] - s_base[remove:]) + np.mean(signal)
    ...
    n = len(reference)
    lin.fit(reference.reshape(n, 1), signal.reshape(n, 1))
    return lin.predict(reference.reshape(n, 1)).reshape(n, )
