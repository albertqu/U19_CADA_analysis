...
def test_model_recovery():
    ...
    model, param_file = BIModel, os.path.join(cache_folder, f'bsd_simopt_params_{data_arg}_BI_v12.csv')
...
def test_model_generate():
    ...
    model_arg = 'BI'
    model = BIModel
    param_file = os.path.join(cache_folder, f'bsd_simopt_params_{data_arg}_{model_arg}_v12.csv')
    params_df = pd.read_csv(param_file)
    ...
    gen_data.to_csv(os.path.join(cache_folder, f'bsd_generated_{data_arg}_{model_arg}_v12.csv'), index=False)
...
def test_model_eckstein2022_RLCF():
    ...
    model = RLCF()
    model_arg = 'RLCF'
    param_ranges = {'st': (0.13, 0.2),
                    'beta': (2.5, 2.8),
                    'a_neg': (0.65, 0.7),
                    'a_pos': (0.71, 0.75)}
    np.random.seed(230)

    test_modelgen_mp(model, model_arg, 'eckstein2022', param_ranges, niters=1000, save_folder=cache_folder)
...
def test_modelgen_mp(model, model_arg, gen_arg, param_ranges, niters=1000, save_folder=None):
    if save_folder is None:
        cache_folder = r"D:\U19\data\Probswitch\caching"
    else:
        cache_folder = save_folder
    params = {k: np.random.random(niters) * (v[1] - v[0]) + v[0] for k, v in param_ranges.items()}
    ...
    with Pool(processes=psutil.cpu_count(logical=False)) as pool:
        result = pool.map_async(model.generate, [params_df.loc[[i]] for i in range(len(params_df))], chunksize=125)

        for gen_data in tqdm(result.get()):
            all_gen.append(gen_data)
    data = pd.concat(all_gen, axis=0).sort_values(['ID', 'Session', 'Trial'])
    data.to_csv(os.path.join(cache_folder, f'gendata_{model_arg}_{gen_arg}.csv'), index=False)
...
def test_modelgen_recover_mp(model, model_arg, gen_arg, param_ranges, niters=1000,
                             save_folder=None, method='trust-constr', ntrial=1000, nsess=2):
    if save_folder is None:
        cache_folder = r"D:\U19\data\Probswitch\caching"
    else:
        cache_folder = save_folder
    ...
    all_params = []
    with Pool(processes=psutil.cpu_count(logical=False)) as pool:
        result = pool.starmap_async(generate_and_recover,
    ...
    params_df.to_csv(os.path.join(cache_folder, f'genrec_{gen_arg}_{model_arg}_{method}_paramsx.csv'))
    params_y.to_csv(os.path.join(cache_folder, f'genrec_{gen_arg}_{model_arg}_{method}_paramsy.csv'))
    data.to_csv(os.path.join(cache_folder, f'genrec_{gen_arg}_{model_arg}_{method}_gendata.csv'))
...
def test_model_genrec_eckstein2022():
    ...
    param_ranges = {'st': (0.07, 0.1),
                    ...
                    'sw': (0.142, 0.152)}
    np.random.seed(230)
    data, params_x, params_y = test_modelgen_recover_mp(model, model_arg, 'eckstein2022', param_ranges,
...
def test_model_genrec_eckstein2022_BIfp():
    ...
    param_ranges = {'st': (0, 1),
                    ...
                    'sw': (0.01, 0.2)}
    np.random.seed(230)
    data, params_x, params_y = test_modelgen_recover_mp(model, model_arg, 'eckstein2022_1s5ht', param_ranges,
                                                        niters=1000, save_folder=cache_folder, method='L-BFGS-B')
...
def test_model_genrec_BSD(model):
    ...
    version = 'v13'
    pfile = os.path.join(cache_folder, f"bsd_simopt_params_{data_arg}_{model_arg}_{version}.csv")
...
def viz_genrec_recovery(model, gen_arg):
    ...
    method = 'L-BFGS-B'
    cache_folder = r"D:\U19\data\Probswitch\caching"
    params_x = pd.read_csv(os.path.join(cache_folder, f'genrec_{gen_arg}_{model_arg}_{method}_paramsx.csv'))
    params_y = pd.read_csv(os.path.join(cache_folder, f'genrec_{gen_arg}_{model_arg}_{method}_paramsy.csv'))

    plist = list(mdl.param_dict.keys())
    px = params_x.melt(id_vars='ID', value_vars=plist, var_name='param', value_name='truth')
    py = params_y.melt(id_vars='ID', value_vars=plist, var_name='param', value_name='fitted')
    ...
    g = sns.lmplot(data=df.groupby('param').apply(drop_outlier).reset_index(drop=True), x='truth', y='fitted',
                   col='param', col_wrap=2, facet_kws={'sharey': False, 'sharex': False},
...
def gendata_behavior_plot(model_arg, gen_arg):
    ...
    return animal_behavior_plot_eckstein(data, plot_folder, f'{model_arg}_gendata_{gen_arg}')
...
def animal_behavior_plot_eckstein(data, plot_folder=None, save_arg=None, title=None):
    ...
    data['trialHistory'] = pd.Categorical(data['trialHistory'], categories=['++', '-+', '+-', '--'], ordered=True)

    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(12, 6))
    sns.lineplot(data=data[data['blockTrial'] <= 10].reset_index(drop=True), x='blockTrial', y='correct',
                 errorbar='se', ax=axes[0])
    sns.barplot(data=data.dropna(subset=['trialHistory']).reset_index(drop=True),
                x='trialHistory', y='stay%', errorbar='se', ax=axes[1], hue_order=['++', '-+', '+-', '--'])
    sns.despine()
...
def replicate_eckstein2022_fit_params():
    ...
    for model_arg in ['RLCF', 'BI', 'PC']:
        version = 'v12'
        params_df = pd.read_csv(os.path.join(cache_folder, f"bsd_simopt_params_{data_arg}_{model_arg}_{version}.csv"))
...
def fit_LR_BSD():
    ...
    pd.concat(all_params, axis=0).sort_values('ID').to_csv(os.path.join(cache_folder,
                                                                        f"bsd_simopt_params_{data_arg}_{model_arg}_{version}.csv"),
                                                    index=False)
...
def test_mp_multiple_animals(model):
    ...
    with Pool(processes=psutil.cpu_count(logical=False)) as pool:
        results = [pool.apply_async(fit_model_per_subject,
                                    args=(data[data['Subject'] == subj].reset_index(drop=True), model)) for subj in
                   data['Subject'].unique()]
...
def test_model_recovery_mp(model, model_arg):
    ...
    data = pd.read_parquet(data_file)
    data['ID'] = data['Subject']
    param_file = os.path.join(cache_folder, f'bsd_simopt_params_{data_arg}_{model_arg}_{version}.csv')
    params_df = pd.read_csv(param_file)
    md = model()
    ...
    with Pool(processes=psutil.cpu_count(logical=False)) as pool:
        results = [pool.apply_async(fit_model_per_subject,
                                    args=(em_data[em_data['Subject'] == subj].reset_index(drop=True), model)) for subj
                   in data['Subject'].unique()]
        for r in tqdm(results):
            id_sim, id_fp = r.get()
            all_data_sim.append(id_sim)
            all_params.append(id_fp)
    ...
    data_sim_opt = pd.concat(all_data_sim, axis=0).sort_values(['Subject', 'Session', 'Trial'])
    data_sim_opt.to_csv(os.path.join(cache_folder, f"bsd_simopt_data_{data_arg}_{model_arg}_recover_{version}.csv"),
                        index=False)
    pd.concat(all_params, axis=0).sort_values('ID').to_csv(os.path.join(cache_folder,
...
def test_multiple_animals():
    ...
    pd.concat(all_params, axis=0).sort_values('ID').to_csv(os.path.join(cache_folder,
                                                                        f"bsd_simopt_params_{data_arg}_{model_arg}_v6.csv"),
                                                           index=False)
...
def compare_models():
    ...
    for model_arg in model_names:
        simfile = os.path.join(cache_folder, f"bsd_simopt_data_{data_arg}_{model_arg}_{version}.csv")
        paramfile = os.path.join(cache_folder, f"bsd_simopt_params_{data_arg}_{model_arg}_{version}.csv")
        simdata = pd.read_csv(simfile)
        param_df = pd.read_csv(paramfile)
        simdata = simdata[simdata['Decision'] != -1].reset_index(drop=True)
        rdf = simdata.groupby('ID').apply(calculate_scores_perID).reset_index(drop=True).merge(
            param_df[['ID', 'aic', 'bic']], on='ID')
        rdf['model'] = model_arg
        results.append(rdf)
...
def viz_test_softmax():
    ...
    test_data = pd.read_csv(os.path.join(cache_folder, f'genrec_{model_arg}_{gen_arg}_gendata.csv'))

    rlm = RL()
    # TODO: handle miss decisions
    data = rlm.fit_marginal(test_data)
    # data.to_csv(os.path.join(cache_folder, 'bsd_sim_sample_RLCF.csv'))
    params_df = pd.read_csv(os.path.join(cache_folder, r'genrec_RLCF_eckstein2022_paramsx.csv'))
    # data_sim.to_csv(os.path.join(cache_folder, "bsd_sim_data_sample.csv"))
    ...
    for i in range(len(v)):
        if i <= 2000:
            ...
            p_annes[i] = softmax(np.array([q0, q1]), params_df.loc[params_df['ID'] == i_id, 'beta'].values[0])[1]
            p_v[i] = qdiff(np.array([q0, q1]), params_df.loc[params_df['ID'] == i_id, 'beta'].values[0])
