...
def inverse_kernel(c, N=None, fft=True):
    ...
    if fft:
        ...
        H = spdiags(
            [np.full(N, ic) for ic in c], np.arange(0, -3, step=-1), format="csc"
        )
        G = sp_linalg.inv(H)
        return G[-1, ::-1]
...
def moving_average(s, window=30, non_overlap=False, pad=False):
    ...
    if non_overlap:
        smoothen = [
            np.mean(s[i : i + window]) for i in range(0, len(s) - window + 1, window)
        ]
    else:
...
class SpikeCalciumizer:
    ...
    fluorescence_saturation = 0.0  # 300.
    alpha = 1.0  # 50 uM
    bl = 0
    tauImg = 100  # ms;
    tauCa = 400.0  # ms
    ...
    def apply_tranform_from_file(
        self, *args, sample=None
    ):  # TODO: add #neurons to simulated spike,
        # last item possibly
    ...
    def binned_spikes_to_calcium(self, neuron_acts, c0=0, fast_inverse=False):
        ...
        if self.AR_order is not None and self.g is not None:
            if fast_inverse:
                G = spdiags(
                    [np.ones(T)] + [np.full(T, -ig) for ig in self.g],
                    np.arange(0, -self.AR_order - 1, step=-1),
                    format="csc",
                )
                ...
                for t in range(1, T):
                    ar_sum = np.sum(
                        [
                            calcium[:, t - i] * self.g[i - 1]
                            for i in range(1, min(t, self.AR_order) + 1)
                        ],
                        axis=0,
                    )
        ...
        if self.fluorescence_saturation > 0:
            calcium = (
                self.fluorescence_saturation
                * calcium
                / (calcium + self.fluorescence_saturation)
            )
        calcium += (
            self.bl
        )  # TODO: determine whether it is better to add baseline before or after saturation
        if self.std_noise:
            multiplier = (
                self.fluorescence_saturation if self.fluorescence_saturation > 0 else 1
            )
...
def signal_filter_visualize(
    FP_415_time, FP_415_signal, FP_470_time, FP_470_signal, isosbestic=True, **kwargs
):
    ...
    if isosbestic:
        ...
        bases = {
            "415_mean": f0,
            "f0_npower_correct": f0_npower_correct,
            "f0_std_correct": f0_std_correct,
        }
        ...
        f0_percAuto = percentile_filter(
            FP_470_time, FP_470_signal + m, perc=None, **kwargs
        )
...
def FP_viz_whole_session(
    ...
    # Find the baseline
    r_base = airPLS(
        smoothened_reference.T, lambda_=lambd, porder=porder, itermax=itermax
    )
    ...
    palettes = {
        "sig": sns.color_palette("icefire")[:2],
    ...
    for i in range(nseq):
        ...
        axes.ravel()[i].plot(
            segment_time, sig_segment, label=ch, lw=all_lw, color=palettes["sig"][0]
        )
        axes.ravel()[i].plot(
            segment_time,
            ref_segment - ref_off,
        ...
        axes.ravel()[i].plot(
            ...
            label=ch + "_trend",
            lw=all_lw,
            color=palettes["sig"][1],
        )
        axes.ravel()[i].plot(
            segment_time,
            rb_segment - ref_off,
            label=control_ch + "_trend",
            lw=all_lw,
            color=palettes["ref"][1],
        )
        axes.ravel()[i].set_ylabel(f"{roi_string}Z(RawF)")
        axes.ravel()[i].set_xlabel(f"Rel. Time ({time_unit})")
        axes.ravel()[i].set_xlim(
            [start - min_time - 0.05 * interval, end - min_time + 0.05 * interval]
        )
        pend = "th"
        if i == 0:
            pend = "st"
...
class ProgressBar:
    ...
    def loop_end(self, task_name):
        ...
        if ETA == 0.0:
            print(
                f"Finished all {self.N} tasks. Total Run Time: {self.tstr(time.time()-self.start)}."
            )
