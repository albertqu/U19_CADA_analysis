...
def get_action_outcome_latencies(mat):
    ...
    ipsi, contra = (
        ("right", "left")
        if np.array(access_mat_with_path(mat, "notes/hemisphere")).item()
        else ("left", "right")
    )
    ipsi_choice_trials = access_mat_with_path(
        mat, f"trials/{ipsi}_in_choice", ravel=True, dtype=np.int
    )
    ipsi_choice_time = access_mat_with_path(mat, f"time/{ipsi}_in_choice", ravel=True)
    contra_choice_trials = access_mat_with_path(
        mat, f"trials/{contra}_in_choice", ravel=True, dtype=np.int
    )
    contra_choice_time = access_mat_with_path(
        mat, f"time/{contra}_in_choice", ravel=True
    )
...
def get_center_port_stay_time(mat):
    ...
    center_out_trial = access_mat_with_path(
        mat, "glml/trials/execute", ravel=True, dtype=np.int
    )
...
def get_trial_outcome_laterality(mat, as_array=False):
    ...
    for side in "ipsi", "contra":
        rew = (
            access_mat_with_path(
                mat, f"glml/trials/{side}_rew", ravel=True, dtype=np.int
            )
            - 1
        )
        unrew = (
            access_mat_with_path(
                mat, f"glml/trials/{side}_unrew", ravel=True, dtype=np.int
            )
            - 1
        )
...
def get_trial_outcomes(mat, as_array=False):
    ...
    if as_array:
        return outcomes
    outcomes = decode_trial_behavior(
        outcomes,
        {"No choice": 3, "Incorrect": 2, "Correct Omission": 1.1, "Rewarded": 1.2},
    )
...
def get_trial_features(mat, feature, as_array=False, drop_empty=True, as_df=False):
    ...
    if not isinstance(mat, BehaviorMat):
        return np.arange(get_trial_num(mat)), get_trial_features_old(
            mat, feature, as_array
        )
    ...
    if ("MLAT" not in features) or ("MLAT_sal" in features):  # trial level features
        ...
        if len(etrials) != mat.trialN:
            assert len(np.unique(etrials)) == len(
                etrials
            ), "duplicates contained in salient only?"
        ...
        trial_event_features = np.full(
            len(etrials), "", dtype=f"<U{(maxlen+1) * len(lags)}"
        )
        all_lag_features = [fpast(efeatures, ilag) for ilag in lags]
        for i in range(len(trial_event_features)):
            ith_features = [
                all_lag_features[il][i] for il in range(len(all_lag_features))
            ]
            if "" in ith_features:
    ...
    if as_array:
        if as_df:
            return pd.DataFrame(
                {
                    "animal": np.full(len(etrials), mat.animal),
                    "session": np.full(len(etrials), mat.session),
                    "behavior_times": trial_event_features,
                    "trial": etrials,
                }
            )
    ...
    if isinstance(trial_event_features[0], str):
        ...
        results = {
            feat: (trial_event_features == feat)
            for feat in np.unique(trial_event_features)
...
def get_trial_features_old(mat, feature, as_array=False):
    ...
    if feature == "R{t-2,t-1}":
        ...
        for oi in outcomes:
            for oj in outcomes:
                results[oi[0] + oj[0]] = np.logical_and.reduce(
                    [fpast(trial_outcomes[oi], -2), fpast(trial_outcomes[oj], -1)]
                )
        ...
        for il in lateralities:
            for jl in lateralities:
                ...
                results[jl + "_" + stay] = np.logical_and.reduce(
                    [
                        fpast(trial_laterality[il], lags[1]),
        ...
        for il in lateralities:
            for jl in lateralities:
                stay = "stay" if (il == jl) else "switch"
                results[jl + "_" + stay] = np.logical_and.reduce(
                    [fpast(trial_laterality[il], 0), fpast(trial_laterality[jl], -1)]
                )
...
def get_behavior_times(mat, behavior, simple=True, saliency=True, as_df=False):
    ...
    if saliency:
        ...
        behavior_trials, behavior_times = (
            np.arange(mat.trialN)[nonans],
            behavior_temp[nonans],
        )
    if as_df:
        return pd.DataFrame(
            {
                "animal": np.full(len(behavior_times), mat.animal),
                "session": np.full(len(behavior_times), mat.session),
                "behavior_times": behavior_times,
                "trial": behavior_trials,
            }
        )
...
def get_animal_session_behavior_dataframe(folder, animal, session):
    files = encode_to_filename(
        folder, animal, session, ["green", "red", "FP", "behavior_old", "processed"]
    )
    ...
    feature_mat = pd.DataFrame(
        np.vstack([rew_feature, side_feature]).T, columns=["R", "A"]
    )
    ...
    header_mat["hemi"] = (
        "right" if np.array(mat["glml/notes/hemisphere"]).item() else "left"
    )
...
class EventNodeDeprecated:
    ...
    def __str__(self):
        return (
            f"EventNode({self.event}, {self.trial}, {self.etime:.1f}ms, {self.ecode})"
        )
...
class BehaviorMatOld(BehaviorMat):
    ...
    code_map = {
        ...
        74: ("outcome", "abort"),
    }  # saliency questionable did not poke enough time?

    fields = [
        "center_in",
        "center_out",
        "side_in",
        "outcome" "zeroth_side_out",
        "first_side_out",
        "last_side_out",
        "reward",
        "action",
    ...
    def initialize(self, hfile):
        ...
        self.exp_complexity[
            0
        ] = False  # TODO: decide where it is fair to ignore exploration before first trial
